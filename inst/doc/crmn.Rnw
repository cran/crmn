...
which features are the internal standards coded by a \texttt{tag}
component which should be equal \texttt{"IS"} for the standards. To use
the \texttt{RI} method the feature data must also contain the
retention index of each analyte. See the dataset \texttt{mix} for an
example.
...
dim(Ya)
@ %
These two functions must work as they should for your data too so
